...
class Wan22Trainer:
    def __init__(self, model, train_dataset, val_dataset=None, *, cfg: DictConfig):
        ...
        if self.keep_last_checkpoints is not None and self.keep_last_checkpoints < 1:
            raise ValueError(
                ...
                f"got {self.keep_last_checkpoints}."
            )
    ...
    def _prune_old_checkpoints(self, target_keep: int | None = None):
        keep = self.keep_last_checkpoints
        if keep is None:
            return []
        if target_keep is not None:
            keep = int(target_keep)
        keep = max(int(keep), 0)
        ...
        if weights_root.exists():
            for path in weights_root.glob("step_*.pt"):
                step = self._checkpoint_step_from_path(path)
                if step is not None:
                    candidates.setdefault(step, {})["weight"] = path
        ...
        if state_root.exists():
            for path in state_root.glob("step_*"):
                ...
                if not (path / "trainer_state.json").is_file():
                    logger.warning(
                        "[ckpt-prune] ignoring incomplete state checkpoint without trainer_state.json: %s",
                        path,
                    )
                    continue
                step = self._checkpoint_step_from_path(path)
                if step is not None:
                    candidates.setdefault(step, {})["state"] = path

        complete_steps = sorted(
            step for step, paths in candidates.items()
            if "weight" in paths and "state" in paths
        )
        stale_steps = complete_steps if keep == 0 else complete_steps[:-keep]
        removed = []
        for step in stale_steps:
            paths = candidates[step]
            for kind in ("weight", "state"):
                path = paths.get(kind)
                ...
                logger.info(
                    "[ckpt-prune] removed old %s checkpoint step=%06d path=%s",
                    kind,
                    step,
                    path,
                )
    ...
    def save_checkpoint(self):
        ...
        self.accelerator.wait_for_everyone()
        if self.accelerator.is_main_process and self.keep_last_checkpoints is not None:
            # Free space before writing the new DeepSpeed state.  A single ZeRO
            # checkpoint can be tens of GB, so waiting until after a successful
            # save would require temporary room for one extra full checkpoint.
            # Keep at least one complete fallback checkpoint if possible.
            self._prune_old_checkpoints(
                target_keep=max(int(self.keep_last_checkpoints) - 1, 1)
            )
        self.accelerator.wait_for_everyone()

        weight_path = os.path.join(self.weights_dir, f"{step_tag}.pt")
        if self.accelerator.is_main_process and os.path.exists(weight_path):
            logger.warning(
                "[ckpt] removing stale/incomplete weight checkpoint before re-saving: %s",
                weight_path,
            )
        ...
        if self.accelerator.is_main_process and os.path.exists(state_path):
            logger.warning(
                "[ckpt] removing stale/incomplete state checkpoint before re-saving: %s",
                state_path,
            )
