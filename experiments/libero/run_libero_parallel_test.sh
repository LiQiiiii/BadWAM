...
run_libero_eval() {
    ...
    export LIBERO_TMUX_SESSION_NAME="$SESSION_NAME"
    EXP_NAME=${EXP_NAME:-""}
    export EXP_NAME
    ...
    # Copy task_list_file into OUTPUT_DIR when it is not already there.
    task_list_dest="$OUTPUT_DIR/$(basename "$task_list_file")"
    if [ "$(readlink -f "$task_list_file")" != "$(readlink -f "$task_list_dest" 2>/dev/null || echo "$task_list_dest")" ]; then
        cp "$task_list_file" "$task_list_dest"
    fi
    task_list_file="$task_list_dest"
    ...
    task_has_result() {
        local suite=$1
        local task_id=$2
        local result_file_pattern="$OUTPUT_DIR/$suite/gpu*_task${task_id}_results.json"
        ls $result_file_pattern 1> /dev/null 2>&1
    }
    ...
    launch_task_on_pane() {
        ...
        tmux send-keys -t $SESSION_NAME:$pane_info "source ~/.bashrc && cd $ROOT_DIR && export EXP_NAME=$EXP_NAME && export PYTHONPATH=\"$ROOT_DIR/src:\${PYTHONPATH:-}\" && \
    ...
    for task_info in "${task_array[@]}"; do
        ...
        # Resume support: if this task already has a result from a previous
        # interrupted run, do not launch it again during the initial wave.
        # The later scheduling loop already has this check; keeping the same
        # guard here makes restarts task-level idempotent.
        if task_has_result "$suite" "$task_id"; then
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] Skipping completed task during initial launch: $suite task_id=$task_id"
