...
def resolve_task_file(output_dir: Path, manager: DictConfig) -> Path:
    """Return the task file used by the parallel LIBERO launcher.

    By default we keep the original behavior and generate the full benchmark
    task list under ``output_dir/tasks.txt``.  Ablation/smoke-test launchers can
    pass ``MULTIRUN.task_file=/path/to/tasks.txt`` to run an explicit subset
    without this manager overwriting it.
    """
    ...
    if task_file_cfg:
        ...
        print(f"\nUsing explicit task list: {task_file}")
        print(f"Total tasks: {len([line for line in task_file.read_text(encoding='utf-8').splitlines() if line.strip()])}")
        return task_file
    return create_task_file(output_dir / "tasks.txt", list(manager.task_suite_names))
...
def main(cfg: DictConfig):
    ...
    task_file = resolve_task_file(output_dir, manager)
